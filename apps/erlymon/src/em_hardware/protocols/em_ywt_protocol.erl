...
%% YwtProtocol: %(..),(\d+):\d+,(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2}),([EW])(\d{3}\.\d{6}),([NS])(\d{2}\.\d{6}),(\d+)?,(\d+),(\d+),(\d+),([^,]+),([-0-9a-fA-F]+).*
...
do_process_data(Data, State = #state{transport = _Transport, socket = Socket, protocol = _Protocol, deviceId = 0}) ->
  em_logger:info("[packet] unit: ip = '~s' data: ~s", [em_inet:resolve(Socket), Data]),
  case parse(Data) of
    {ok, Type, Imei, PositionModel, ReportId} ->
      ...
      case em_data_manager:get_device_by_uid(Imei) of
        ...
        {ok, Object} ->
          ...
          do_send_response({Type, ReportId}, State),
          {noreply, State, ?TIMEOUT}
      end;
    {error, Message} ->
...
do_process_data(Data, State = #state{transport = _Transport, socket = Socket, protocol = _Protocol, deviceId = DeviceId}) ->
  em_logger:info("[packet] unit: ip = '~s' data: ~s", [em_inet:resolve(Socket), Data]),
  case parse(Data) of
    {ok, Type, Imei, PositionModel, ReportId} ->
...
do_send_response({Type, ReportId}, #state{transport = Transport, socket = Socket}) ->
  Transport:send(Socket, do_format_response(Type, ReportId)).

do_format_response(Type, ReportId) when (Type =:= <<"KP">>) or (Type =:= <<"EP">>) ->
  list_to_binary(io_lib:format("$AT+~s=~s\r\n", [Type, ReportId])).

do_save_position(#state{socket = Socket, protocol = Protocol}, DeviceId, Imei, PositionModel) ->
  em_logger:info("[packet] unit: ip = '~s' imei = '~s' message: ~w", [em_inet:resolve(Socket), Imei, PositionModel]),
...
%% echo "35. ywt"
%% (echo -n -e "%RP,3000012345:0,090807182815,E114.602345,N22.069725,,30,160,4,0,00\n";) | nc -v localhost 5035
parse(Data) ->
  case em_regexp:match(Data, ?PATTERN) of
    {ok, [_, Type, Imei, Year, Month, Day, Hour, Minute, Second, LonHem, LonDeg, LatHem, LatDeg, Altitude, Speed, Course, Satellite, ReportId | _]} ->
      Position = #position{
        deviceTime = parse_date(Year, Month, Day, Hour, Minute, Second),
        latitude = parse_coordinate(hem_deg, {LatHem, LatDeg}),
        longitude = parse_coordinate(hem_deg, {LonHem, LonDeg}),
        altitude = parse_altitude(Altitude),
        speed = parse_speed(Speed),
        course = parse_course(Course),
        valid = parse_valid(parse_satellite(Satellite)),
        attributes = #{
          ?KEY_SATELLITES => parse_satellite(Satellite)
        }
      },
      {ok, Type, Imei, Position, ReportId};
    Reason ->
      Reason
  end.
...
parse_coordinate(hem_deg, {Hem, Deg}) ->
  hemisphere(list_to_float(binary_to_list(Deg)), Hem).
...
parse_satellite(Satellite) ->
  list_to_integer(binary_to_list(Satellite)).
...
parse_valid(Valid) when Valid >= 3 ->
  true;
parse_valid(_) ->
  false.
...
%%test() ->
%%  Packet = <<"%RP,3000012345:0,090807182815,E114.602345,N22.069725,,30,160,4,0,00\n">>,
