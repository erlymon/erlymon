...
%% Xt013Protocol: (?:HI,\d+)?TK,(\d+),(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2}),([+-]\d+\.\d+),([+-]\d+\.\d+),(\d+),(\d+),\d+,(\d+),([FL]),\d+,(\d+),[0-9a-fA-F]+,[0-9a-fA-F]+,(\d+),[^,]*,(\d+\.\d+),(\d),.*
...
%% echo "76. xt013"
%% (echo -n -e "TK,862950021650364,150131090859,+53.267863,+5.767363,0,38,12,0,F,204,08,C94,336C,24,,4.09,1,,,,,,,,\n";) | nc -v localhost 5076
parse(Data) ->
  case em_regexp:match(Data, ?PATTERN) of
    {ok, [_, Imei, Year, Month, Day, Hour, Minute, Second, Lat, Lon, Speed, Course, Altitude, GpsFix, GpsLvl, _GsmLvl, Battery, Charging | _]} ->
      Position = #position{
        ...
        valid = parse_valid(GpsFix),
        attributes = #{
          ?KEY_GPS => parse_gps_lvl(GpsLvl),
          ?KEY_BATTERY => parse_battery(Battery),
          ?KEY_CHARGE => parse_charge(Charging)
        }
      },
      {ok, Imei, Position};
    Reason ->
...
parse_coordinate(Deg) ->
  list_to_float(binary_to_list(Deg)).

parse_altitude(void) ->
  undefined;
parse_altitude(Altitude) ->
  em_logger:info("ALT: ~s ~w", [Altitude, Altitude]),
...
%%test() ->
%%  Packet = <<"TK,862950021650364,150131090859,+53.267863,+5.767363,0,38,12,0,F,204,08,C94,336C,24,,4.09,1,,,,,,,,\n">>,
